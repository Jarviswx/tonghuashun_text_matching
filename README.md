...
(1)对抗训练：例如目前已经开源的FGM和PGD（FreeAT/YOPO/FreeLB等方法从论文看来效果更优，但是目前还没有开源代码，而其他人复现的方案很难达到论文的效果，故暂时不采用），其中FGM使用起来更方便且更快（训练时间大致为PGD的50%-70%），两者的效果差距也并不明显，故这里我们采用FGM作为对抗训练的方法。

(2)数据增强：大致分为单词级、句子级和生成式三类增强方法；笔者在前期尝试了句子级方法中的交叉增强法，发现模型过拟合严重，于是放弃了数据增强这个trick。（时间关系所以并没有尝试其他方法，或许这里也是一个潜在的涨分点）
